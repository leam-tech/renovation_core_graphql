...
def get_docs_assigned_to_user_resolver(obj, info: GraphQLResolveInfo, **kwargs):
    ...
    for todo in r:
        ...
        todo.assigned_by = _get_user_for_gql(todo.assignedBy)
        todo.assigned_to = _get_user_for_gql(todo.assignedTo)
...
def assign_doc_resolver(obj, info: GraphQLResolveInfo, **kwargs):
    ...
    for todo in r:
        ...
        todo.assigned_to = _get_user_for_gql(todo.owner)
        todo.assigned_by = _get_user_for_gql(todo.assigned_by)
...
def unassign_doc_resolver(obj, info: GraphQLResolveInfo, **kwargs):
    ...
    for todo in r:
        ...
        todo.doctype = None
        todo.due_date = todo.date
        todo.assigned_to = _get_user_for_gql(name=todo.owner)
        todo.assigned_by = _get_user_for_gql(name=todo.assigned_by)
...
def _get_user_for_gql(user: str):
    user_doc = frappe.get_doc("User", user)
    user_doc.apply_fieldlevel_read_permissions()

    return user_doc.as_dict(convert_dates_to_str=True)
